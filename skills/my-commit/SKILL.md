---
name: my-commit
description: 快速检查、暂存并提交当前 Git 仓库的改动。用户调用 $my-commit，或明确要求提交、创建 commit、整理并提交当前修改时使用；默认走低轮次快路径，仅在混合改动、敏感文件、submodule 或暂存边界不清时升级检查。
---
...
3. **暂存并提交**：优先显式路径；确认整个工作树属于同一任务时可用 `git add -A`。已完整审阅同一批路径且没有既有 staged 冲突时，可在一次工具调用中顺序执行暂存、`git diff --cached --check`、commit。
...
- 改动跨多个目的，包含 submodule/gitlink、重命名、删除、冲突或大量生成文件。
